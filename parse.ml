...
let transpose (grid : char array array) : char array array =
  ...
  let num_cols = Array.length grid in
...
let update_colors (colors : (char, int) Hashtbl.t) (new_elem : char) =
  let (r, g, b) = (Random.int 256, Random.int 256, Random.int 256) in
  let color = (r lsl 16) + (g lsl 8) + b in
  Hashtbl.add colors new_elem color;
  ;;

(* Utility function to make an individual tile *)
let make_tile (shape : char array array) (colors : (char, int) Hashtbl.t) : (tile * ((char, int) Hashtbl.t)) =
  let result = Array.make (Array.length shape) (Array.make 0 0) in
  Array.iteri
    ( fun idx elem ->
          ( let row = Array.make (Array.length elem) 0 in
            Array.iteri
...
let rec make_tiles (shapes : char array array list) (colors : (char, int) Hashtbl.t) : tile list =
  match shapes with
  | shape :: rest ->
...
let parse_grid (raw_grid : char array array) : ((tile list) * board) =
  ...
  let blank_cols = Array.fold_lefti
    (fun acc idx elem -> if Array.forall (fun x -> x = '.') elem
                            then acc @ [idx]
                            else acc)
    []
    trans_grid
  ...
  let bounds = Array.fold_lefti
    (fun idx elem -> if idx + 1 < (Array.length blank_cols) &&
  ...
  Hashtbl.add color_table '.' 0x000000;
  let tile_set = make_tiles tile_arrays color_table in
  let dimensions = List.map
    (fun elem -> Array.fold_left (fun acc r_elem -> acc + (Array.length r_elem)) 0 elem)
    tile_set
    in
  let largest_size = List.max dimensions in
  let (largest_size_idx, _) = List.findi (fun _ x -> x = largest_size) dimensions in
  let largest_tile = List.findi (fun idx x -> idx = largest_size_idx) tile_set in
  let tile_set = List.remove_at tile_set largest_size_idx in
  (tile_set, largest_tile)
  ;;
...
let read_input (file_name : string) : ((tile list) * (board)) =
  ...
  let input_grid = make_grid input in
  close_in input;
